...
local function drawScreen()
  ...
  if #state.ports == 0 then
    at(
      L,
      cur,
      "No processor ports found in recipes.",
      colors.gray,
      colors.black
    )
  else
  ...
  mkBtn(L, H, "Replace All", colors.black, colors.orange, function()
    ...
    for _, entry in ipairs(state.ports) do
      if entry.newLabel then
        portToLabel[entry.value] = entry.newLabel
      end
    end
    ...
    for _, recipe in pairs(recipes) do
      local function rep(v)
        if v and portToLabel[v] then
          count = count + 1
          return portToLabel[v]
        end
        return v
      end
...
for _, name in ipairs(perifs) do
  table.insert(
    state.labelItems,
    { name = name, label = labelsData[name] or "" }
  )
end
